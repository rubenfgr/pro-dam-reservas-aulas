...
public class Reservas {
	...
	public void insertar(Reserva reserva) throws OperationNotSupportedException {
		...
		Reserva res = buscar(reserva);
		if (res != null) {
			if (res.getPermanencia() instanceof PermanenciaPorTramo) {
				throw new OperationNotSupportedException("La reserva para ese aula y ese tramo ya existe.");
			} else {
				throw new OperationNotSupportedException("La reserva para ese aula y esa hora ya existe.");
			}
		} else {
...
//				eq(MongoDB.PROFESOR_NOMBRE, reserva.getProfesor().getNombre()),
	...
	public void borrar(Reserva reserva) throws OperationNotSupportedException {
		...
		if (buscar(reserva) != null) {
			...
			coleccionReservas.deleteOne(and(eq(MongoDB.AULA_NOMBRE, reserva.getAula().getNombre()),
					eq(MongoDB.PERMANENCIA_DIA, reserva.getPermanencia().getDia().format(MongoDB.FORMATO_DIA)),
					filtroPermanencia));
		} else {
	...
	public List<Reserva> getReservasPermanencia(Permanencia permanencia) {
		...
		if (permanencia instanceof PermanenciaPorHora) {
			filtroPermanencia = eq(MongoDB.PERMANENCIA_HORA,
					((PermanenciaPorHora) permanencia).getHora().format(MongoDB.FORMATO_HORA));
		} else {
			filtroPermanencia = eq(MongoDB.PERMANENCIA_TRAMO, ((PermanenciaPorTramo) permanencia).getTramo().name());
		}
		for (Document documentoReserva : coleccionReservas.find().filter(and(
				eq(MongoDB.PERMANENCIA_DIA, permanencia.getDia().format(MongoDB.FORMATO_DIA)), filtroPermanencia))) {
	...
	public boolean consultarDisponibilidad(Aula aula, Permanencia permanencia) {
		...
		Reserva reserva = new Reserva(new Profesor("a", "s@d.f"), aula, permanencia);
		if (buscar(reserva) != null) {
			return false;
		}
		return true;
